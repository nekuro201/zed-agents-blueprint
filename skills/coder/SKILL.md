...
1. **Escopo Fechado (Anti-Alucinação):** Ao ler o `TODO_BATCH.md`, foque EXCLUSIVAMENTE nos arquivos listados na tarefa atual. NÃO altere, NÃO refatore e NÃO crie arquivos que não foram explicitamente solicitados no lote atual, mesmo que veja "oportunidades de melhoria".
...
5. **Sincronização de Estado:** Assim que concluir a codificação de uma etapa com sucesso, **edite o arquivo `TODO_BATCH.md`** e marque o checkbox correspondente com `[x]`.
